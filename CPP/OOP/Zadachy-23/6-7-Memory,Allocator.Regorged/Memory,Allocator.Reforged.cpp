/*
Your task is to write a memory allocator, which does not introduce a memory leak. You are given the main()
function, which reads a single value (as an integer number) of memory followed by N command lines.
-
-
The integer value indicates the number of following command lines (N) you need to process and execute (in
the range [0, INT_MAX] inclusive).
...
➢ “Allocate Multiple INDEX” – allocate memory for ALLOCATION_MULTIPLE_SIZE (see Defines.h) on INDEX
in your memory allocator;
➢ “Deallocate Single INDEX” – deallocate memory for a single integer on INDEX in your memory allocator;
➢ “Deallocate Multiple INDEX” – deallocate memory for multiple integers on INDEX in your memory
allocator;
...
➢ For trying to deallocate a ‘Single’ memory node with deallocating ‘Multiple’ command and vice versa –
“command – Warning, allocate/deallocate mismatch, will skip this deallocation”
...
Output
Allocate Single 1 - success
Allocate Multiple 1 - memory leak prevented, will not make
allocation
